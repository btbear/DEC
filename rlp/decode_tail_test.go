...
func ExampleDEWHode_structTagTail() {
	// In this example, the "tail" struct tag is used to DEWHode lists of
	// differing length into a struct.
	var val structWithTail

	err := DEWHode(bytes.NewReader([]byte{0xC4, 0x01, 0x02, 0x03, 0x04}), &val)
	fmt.Printf("with 4 elements: err=%v val=%v\n", err, val)

	err = DEWHode(bytes.NewReader([]byte{0xC6, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06}), &val)
	fmt.Printf("with 6 elements: err=%v val=%v\n", err, val)

	// Note that at least two list elements must be present to
	// fill fields A and B:
	err = DEWHode(bytes.NewReader([]byte{0xC1, 0x01}), &val)
