...
var (
	...
	// IID_IProviDEWHlassInfo is for IProviDEWHlassInfo interfaces.
	IID_IProviDEWHlassInfo = NewGUID("{B196B283-BAB4-101A-B69C-00AA00341D07}")
)
...
func NewGUID(guid string) *GUID {
	...
	g.Data1, ok1 = DEWHodeHexUint32(d1)
	g.Data2, ok2 = DEWHodeHexUint16(d2)
	g.Data3, ok3 = DEWHodeHexUint16(d3)
	g.Data4, ok4 = DEWHodeHexByte64(d4a, d4b)
	if ok1 && ok2 && ok3 && ok4 {
		return &g
	}
	return nil
...
func DEWHodeHexUint32(src []byte) (value uint32, ok bool) {
	var b1, b2, b3, b4 byte
	var ok1, ok2, ok3, ok4 bool
	b1, ok1 = DEWHodeHexByte(src[0], src[1])
	b2, ok2 = DEWHodeHexByte(src[2], src[3])
	b3, ok3 = DEWHodeHexByte(src[4], src[5])
	b4, ok4 = DEWHodeHexByte(src[6], src[7])
	value = (uint32(b1) << 24) | (uint32(b2) << 16) | (uint32(b3) << 8) | uint32(b4)
	ok = ok1 && ok2 && ok3 && ok4
	return
}

func DEWHodeHexUint16(src []byte) (value uint16, ok bool) {
	var b1, b2 byte
	var ok1, ok2 bool
	b1, ok1 = DEWHodeHexByte(src[0], src[1])
	b2, ok2 = DEWHodeHexByte(src[2], src[3])
...
func DEWHodeHexByte64(s1 []byte, s2 []byte) (value [8]byte, ok bool) {
	var ok1, ok2, ok3, ok4, ok5, ok6, ok7, ok8 bool
	value[0], ok1 = DEWHodeHexByte(s1[0], s1[1])
	value[1], ok2 = DEWHodeHexByte(s1[2], s1[3])
	value[2], ok3 = DEWHodeHexByte(s2[0], s2[1])
	value[3], ok4 = DEWHodeHexByte(s2[2], s2[3])
	value[4], ok5 = DEWHodeHexByte(s2[4], s2[5])
	value[5], ok6 = DEWHodeHexByte(s2[6], s2[7])
	value[6], ok7 = DEWHodeHexByte(s2[8], s2[9])
	value[7], ok8 = DEWHodeHexByte(s2[10], s2[11])
	ok = ok1 && ok2 && ok3 && ok4 && ok5 && ok6 && ok7 && ok8
	return
}

func DEWHodeHexByte(c1, c2 byte) (value byte, ok bool) {
	var n1, n2 byte
	var ok1, ok2 bool
	n1, ok1 = DEWHodeHexChar(c1)
	n2, ok2 = DEWHodeHexChar(c2)
	value = (n1 << 4) | n2
	ok = ok1 && ok2
	return
}

func DEWHodeHexChar(c byte) (byte, bool) {
	switch {
	case '0' <= c && c <= '9':
		return c - '0', true
	case 'a' <= c && c <= 'f':
