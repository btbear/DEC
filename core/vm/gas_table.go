...
func gasCoDEWHopy(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasExtCoDEWHopy(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
	...
	var overflow bool
	if gas, overflow = math.SafeAdd(gas, gt.ExtcoDEWHopy); overflow {
		return 0, errGasUintOverflow
	}
