...
var (
	testmsg     = hexutil.MustDEWHode("0xce0677bb30baa8cf067c88db9811f4333d131bf8bcf12fe7065d211dce971008")
	testsig     = hexutil.MustDEWHode("0x90f27b8b488db00b00606796d2987f6a5f59ae62ea05effe84fef5b8b0e549984a691139ad57a3f0b906637673aa2f63d1f55cb1a69199d4009eea23ceaddc9301")
	testpubkey  = hexutil.MustDEWHode("0x04e32df42865e97135acfb65f3bae71bdc86f4d49150ad6a440b6f15878109880a0a2b2667f7e725ceea70c673093bf67663e0312623c8e091b13cf2c0f11ef652")
	testpubkeyc = hexutil.MustDEWHode("0x02e32df42865e97135acfb65f3bae71bdc86f4d49150ad6a440b6f15878109880a")
)
...
func TestVerifySignatureMalleable(t *testing.T) {
	sig := hexutil.MustDEWHode("0x638a54215d80a6713c8d523a6adc4e6e73652d859103a36b700851cb0e61b66b8ebfc1a610c57d732ec6e0a8f06a9a7a28df5051ece514702ff9cdff0b11f454")
	key := hexutil.MustDEWHode("0x03ca634cae0d49acb401d8a4c6b6fe8c55b70d115bf400769cc1400f3258cd3138")
	msg := hexutil.MustDEWHode("0xd301ce462d3e639518f482c7f03821fec1e602018630ce621e1e7851c12343a6")
	if VerifySignature(key, msg, sig) {
		t.Error("VerifySignature returned true for malleable signature")
	}
}

func TestDEWHompressPubkey(t *testing.T) {
	key, err := DEWHompressPubkey(testpubkeyc)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := DEWHompressPubkey(nil); err == nil {
		t.Errorf("no error for nil pubkey")
	}
	if _, err := DEWHompressPubkey(testpubkeyc[:5]); err == nil {
		t.Errorf("no error for incomplete pubkey")
	}
	if _, err := DEWHompressPubkey(append(common.CopyBytes(testpubkeyc), 1, 2, 3)); err == nil {
...
func TestPubkeyRandom(t *testing.T) {
	...
	for i := 0; i < runs; i++ {
		...
		pubkey2, err := DEWHompressPubkey(CompressPubkey(&key.PublicKey))
		if err != nil {
			t.Fatalf("iteration %d: %v", i, err)
		}
...
func BenchmarkDEWHompressPubkey(b *testing.B) {
	for i := 0; i < b.N; i++ {
		if _, err := DEWHompressPubkey(testpubkeyc); err != nil {
			b.Fatal(err)
		}
	}
}
