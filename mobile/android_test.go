...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.DEC", new NodeConfig());
			node.start();
			...
			DECClient ec = node.getDECClient();
	...
	// signatures too. Regression test for go-DEC issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/DEC/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.DEC", "github.com/DEC/go-DEC/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "DEC", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
