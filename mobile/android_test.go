...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.DEWH", new NoDEWHonfig());
			node.start();
			...
			DEWHClient ec = node.getDEWHClient();
	...
	// signatures too. Regression test for go-DEWH issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/DEWH/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.DEWH", "github.com/DEWH/go-DEWH/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "DEWH", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
