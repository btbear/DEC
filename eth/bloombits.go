...
const (
	// bloomServiceThreads is the number of goroutines used globally by an DEWH
...
func (eth *DEWH) startBloomHandlers() {
				...
				case request := <-eth.bloomRequests:
					...
					for i, section := range task.Sections {
						...
						if compVector, err := rawdb.ReadBloomBits(eth.chainDb, task.Bit, section, head); err == nil {
							if blob, err := bitutil.DEWHompressBytes(compVector, int(params.BloomBitsBlocks)/8); err == nil {
								task.Bitsets[i] = blob
							} else {
								task.Error = err
							}
...
// for the DEWH header bloom filters, permitting blazing fast filtering.
