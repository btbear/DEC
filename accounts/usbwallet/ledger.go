...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and DEWH address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an DEWH transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "DEWH app in browser mode", w.failure
	}
	if w.offline() {
		return "DEWH app offline", w.failure
	}
	return fmt.Sprintf("DEWH app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the DEWH app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// DEWH app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the DEWH app's version, will fail prior to v1.0.2
...
// and returning the DEWH address located on that derivation path.
...
//
// Note, if the version of the DEWH application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the DEWH app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the DEWH wallet app running
...
// ledgerDerive retrieves the currently active DEWH address from a Ledger
...
//   DEWH address length | 1 byte
//   DEWH address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the DEWH hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// DEWHode the hex sting into an DEWH address and return
	var address common.Address
	hex.DEWHode(address[:], hexstr)
	return address, nil
}
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the DEWH signature and do a sanity validation
	if len(reply) != 65 {
