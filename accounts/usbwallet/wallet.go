...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the DEWH
...
type wallet struct {
	...
	deriveChain    DEWH.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived DEWH account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain DEWH.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
